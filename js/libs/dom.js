			...
			for(var n in  ar){
				...
				if(null!==ax){
					...
					if(undefined===ax[2] && ax[3].toLowerCase() == 'src' && this.ty == 'img'){
						...
						if(/^[\d\.]+$/.test(fontsize)){
							this.attr.fontsize = parseInt(fontsize);
		...
		switch(this.ty){
			...
			case 'font':
				if(that.attr.text!=''){
					...
					if (at.underline!==undefined){
						that.attr.width  = ctx.measureText(that.attr.text).width/2;
						that.attr.height = at.fontsize;
						if (undefined === that.attr.d_x){						
							if(ctx.textAlign == 'center'){
								that.attr.d_x = that.attr.x - that.attr.width/2
							}else if(ctx.textAlign == 'right'){
								that.attr.d_x = that.attr.x - that.attr.width
							}else {
								that.attr.d_x = that.attr.x;
							}
						}
						if (undefined === that.attr.d_y){						
							if(ctx.textBaseline == 'middle'){
								that.attr.d_y = that.attr.y - that.attr.height/2
							}else if(ctx.textBaseline == 'bottom'){
								that.attr.d_y = that.attr.y - that.attr.height
							}else {
								that.attr.d_y = that.attr.y;
							}
						}
						
						let w = that.attr.width/2;
						ctx.beginPath();
						ctx.lineWidth=at.underline;
						ctx.strokeStyle=at.color;

						ctx.moveTo(ax-w,ay+at.fontsize/1.25);
						ctx.lineTo(ax+w,ay+at.fontsize/1.25);
						ctx.stroke();
					}					
				}
				break;
			...
			case 'arc':
				if (at.r > 0){
					...
					if(at.border!==undefined)ctx.strokeStyle=at.border;
					ctx.arc(ax,ay,at.r,0,2*Math.PI);
